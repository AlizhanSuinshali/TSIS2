...
def main():
    ...
    def save_state():
        """Сохраняет текущее состояние холста в историю (максимум 20 шагов)"""
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_z and ctrl_held:
                    if history: # Если история не пуста
                        canvas.blit(history.pop(), (0, 0))
                    continue

                if typing:
                    if event.key == pygame.K_RETURN:
                        save_state() # Сохраняем состояние перед вставкой текста
                ...
                # Обработка Z без Ctrl (Размер кисти)
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1: 
                    if tool == 'text':
                        ...
                        save_state() # Сохраняем перед заливкой
                        flood_fill(canvas, event.pos[0], event.pos[1], get_color(mode))
                    else:
                        typing = False
                        save_state() # Сохраняем перед началом рисования линии/фигуры
                        drawing = True
                        start_pos = event.pos
                        last_pos = event.pos
